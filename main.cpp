...
int algorithm6(int **, int **, int, int);
int algorithm5(int **, int **, int, int);
...
int algorithm4(int **M, int **L, int numberofvertex, int numberofcolors) {
  int *l = new int[numberofvertex];
  for (int i = 0; i < numberofvertex; i++) {
    l[i] = 0;
  }
...
int algorithm5(int **M, int **L, int numberofvertex, int numberofcolors) {
  int ***backupM = new int **[numberofvertex];
  for (int i = 0; i < numberofvertex; i++) {
    backupM[i] = new int *[numberofvertex];
    for (int j = 0; j < numberofvertex; j++) {
      backupM[i][j] = new int[numberofvertex];
    }
  }
  int ***backupL = new int **[numberofvertex];
  for (int i = 0; i < numberofvertex; i++) {
    backupL[i] = new int *[numberofvertex];
    for (int j = 0; j < numberofvertex; j++) {
      backupL[i][j] = new int[numberofcolors];
    }
  }
  int *l = new int[numberofvertex];
  int *usedVertex = new int[numberofvertex];
  int *usedColors = new int[numberofvertex];
  for (int i = 0; i < numberofvertex; i++)
    l[i] = 0;
  int minPofL = numberofcolors + 1, maxDeg = 0, minP = numberofvertex + 1,
      w = 0, t, t1, t2, x = 0, y, z = 0, s, o = 0, r = 0, newColorMark = 0;
  int *PofL, *Deg, *P, *a, *b, *c;
  for (int i = 0; i < numberofvertex; i++) {
    for (int j = 0; j < numberofvertex; j++) {
    ...
    for (int u = 0; u < numberofcolors; u++)
      for (int j = 0; j < numberofvertex; j++)
        P[u] = P[u] + L[j][u];
    if (newColorMark == 0 && i > 2 &&
        backupL[i - 2][usedVertex[i - 1]][usedColors[i - 1]]) {
      for (int j = 0; j < numberofvertex; j++) {
        for (int k = 0; k < numberofvertex; k++) {
          M[j][k] = backupM[i - 2][j][k];
        }
      }
      for (int j = 0; j < numberofvertex; j++) {
        for (int k = 0; k < numberofcolors; k++) {
          L[j][k] = backupL[i - 2][j][k];
        }
      }
      t1 = usedVertex[i - 1];
      t2 = usedColors[i - 1];
...
int algorithm6(int **M, int **L, int numberofvertex, int numberofcolors) {
  int ***backupM = new int **[numberofvertex];
  for (int i = 0; i < numberofvertex; i++) {
    backupM[i] = new int *[numberofvertex];
    for (int j = 0; j < numberofvertex; j++) {
